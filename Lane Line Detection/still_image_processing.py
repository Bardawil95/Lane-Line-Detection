...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    line_img = np.zeros((int(img.shape[0]), int(img.shape[1]), 3), dtype=np.uint8)
    draw_lines(line_img, lines)
    return line_img

# Python 3 has support for cool math symbols.

def weighted_img(img, initial_img, p1= 0.8, p2 = 1.0, p3 = 0.0):
    """
    ...
    initial_img * p1 + img * p2 + p3
    NOTE: initial_img and img must be the same shape!
    """
    return cv2.addWeighted(initial_img, p1, img, p2, p3)
...
def process_frame(image):
    ...
    result = weighted_img(line_image, image, p1=0.8, p2=1., p3=0.)
    return result
'''
