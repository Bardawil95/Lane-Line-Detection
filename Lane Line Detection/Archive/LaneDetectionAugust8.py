...
cap = cv2.VideoCapture("Dataset7-2.mp4")
...
def pV(image):
    ...
    contours_step2, hierarchy = cv2.findContours(threshYWC, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    ...
    #cv2.imshow("Step 8: Lines inside screen but not toward center ", threshYWC)
...
while 1:
    ...
    cv2.imshow("(7) Original (ROI)", frame)
