...
cap = cv2.VideoCapture("Datasets/Dataset8.avi")
...
def pV(image):
    ...
    blue, green, red = laneFuncs.mostColor(roi_image_most)
    ...
    cv2.imshow("Black & White Frame", temp)
    ...
    for c in contours_step2:
        ...
        i += 1
    #cv2.imshow("(1) Small Objects Elimination", threshYWC)
    ...
    for c in contours_step3:
        ...
        if areaArr_step3[i] > area_tmp or y_step4[i] < yThresh:  # Elimination of non-rectangular objects and removing horizontal lines (y is almost zero)
            cv2.drawContours(threshYWC, [c], 0, black, 30)
            cv2.fillPoly(threshYWC, [c], black)
            #cv2.line(roi_image, tuple(topmost_step4[i]), tuple(bottommost_step4[i]), (0, 0, 255), 15)       # Red color
        else:
            cv2.drawContours(threshYWC, [c], 0, white, 3)
            #cv2.line(roi_image, tuple(topmost_step4[i]), tuple(bottommost_step4[i]), (0, 255, 0), 15)       # Green color
    ...
    #cv2.imshow("(2) Non-rectangular objects & horizontal lines elimination", threshYWC)
   ...
   # cv2.imshow("(3) Vertical Objects but and away from center ", threshYWC)
   ...
   # cv2.imshow("(4) Lines outside screen ", threshYWC)
    ...
    for c in contours_step7:
        ...
        if wholeH < lowH or wholeH > highH:
            print "Deleted Height = ", wholeH
            cv2.drawContours(threshYWC, [c], 0, black, 30)
            cv2.fillPoly(threshYWC, [c], black)
            #print "Whole Height = ", wholeH
            #time.sleep(1)
        else:
            print "Kept Height = ", wholeH
    ...
    roi_image_final = roi_image.copy()
    alphatemp = 0
    itmp = -1
    green = (0, 255, 0)
    ...
    if itmp != -1:
        if distance_step8[itmp] < distThresh:  # If the left line is dotted (Black color)
            cv2.line(roi_image_final, tuple(topmost_step8[itmp]), tuple(bottommost_step8[itmp]), green, 15)
            cv2.line(image, tuple(topmost_step8[itmp]), tuple(bottommost_step8[itmp]), green, 15)
            left = "Dotted"
        else:  # If the left line is continuous (Blue color)
            cv2.line(roi_image_final, tuple(topmost_step8[itmp]), tuple(bottommost_step8[itmp]), blue, 15)
            cv2.line(image, tuple(topmost_step8[itmp]), tuple(bottommost_step8[itmp]), blue, 15)
            left = "Continuous"
    else:
        left = "Unknown"
    ...
    cv2.imshow("Final Masked Frame ", threshYWC)
    ...
    return roi_image, img_gray, threshYWC, current_status, roi_image_final #roi_image_copy, roi_image_most, threshYW, yellow_mask, threshYWC, white_mask
...
while 1:
    ...
    if frame is None or ch & 0xFF == ord('q'):
        break
    roi_image, img_gray, threshYWC, current_status, roi_image_final = pV(frame)
    #threshYWC = laneFuncs.rescale_frame(threshYWC, 40)
    roi_image = laneFuncs.rescale_frame(roi_image, 40)
    frame = laneFuncs.rescale_frame(frame, 60)
    roi_image_final = laneFuncs.rescale_frame(roi_image_final, 40)
    ...
    if status is -1:
        laneFuncs.textDisplay(roi_image_final, "Undefined", white)
        laneFuncs.textDisplay(frame, "Undefined", blue)
    elif status is 0:
        laneFuncs.textDisplay(roi_image_final, "Unknown", white)
        laneFuncs.textDisplay(frame, "Unknown", blue)
    elif status is 1:
        laneFuncs.textDisplay(roi_image_final, "Left", white)
        laneFuncs.textDisplay(frame, "Left", blue)
    elif status is 2:
        laneFuncs.textDisplay(roi_image_final, "Middle", white)
        laneFuncs.textDisplay(frame, "Middle", blue)
    elif status is 3:
        laneFuncs.textDisplay(roi_image_final, "Right", white)
        laneFuncs.textDisplay(frame, "Right", blue)
    #cv2.imshow("RAW ROI", roi_image)
    cv2.imshow("Main frame", frame)
    #cv2.imshow("ROI image", roi_image_final)
    #cv2.imshow("(6) Final Mask", threshYWC)
