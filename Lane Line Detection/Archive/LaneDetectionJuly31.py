...
def pV(image):
    ...
    textDisplay(roi_image, position(right, left), white)

    return roi_image, roi_image_copy, roi_image_most, threshYW, yellow_mask, threshYWC, white_mask, img_gray
...
while 1:
    ...
    if frame is None or ch & 0xFF == ord('q'):
        break
    roi_image, roi_image_copy, roi_image_most, threshYW, yellow_mask, threshYWC, white_mask, img_gray,  = pV(frame)
    threshYWC = rescale_frame(threshYWC, 50)
    roi_image = rescale_frame(roi_image, 50)
    threshYW = rescale_frame(threshYW, 50)
    roi_image_copy = rescale_frame(roi_image_copy, 50)
    img_gray = rescale_frame(img_gray, 50)
    frame = rescale_frame(frame, 80)
    yellow_mask = rescale_frame(yellow_mask, 50)
    white_mask = rescale_frame(white_mask, 50)
