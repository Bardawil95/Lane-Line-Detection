...
def pV(image):
    ...
    for c in contoursnew:
        ...
        y = math.fabs(bottommost[i][1] - topmost[i][1])  # y-coordinate distance for object
        x = math.fabs(bottommost[i][0] - topmost[i][0])  # x-coordinate distance for object
       ...
       # print ">>>[(", bottommost[i][0], ",", bottommost[i][1], ")(", topmost[i][0], ",", topmost[i][1], ")], DX=", x, " DY=", y, "Distance[i]=", distance[i]
        ...
        if areaArr2[i] > area2 or y < yThresh:  # Elimination of non-rectangular objects and removing horizontal lines (y is almost zero)
            cv2.drawContours(threshYWC, [c], 0, black, 30)
            np.delete(contoursnew, c)
            np.delete(distance, i)
            np.delete(bottommost, i)
            np.delete(topmost, i)
        else:
            cv2.drawContours(threshYWC, [c], 0, white, 3)
            #triangles.append(triobj(bottommost[i][0], bottommost[i][1], topmost[i][0], distance[i], 0.6 * image.shape[0], centerImage[0]))
           # cv2.line(roi_image, tuple(topmost[i]), tuple(bottommost[i]), (0, 255, 0), 15)
        i += 1
    i = 0
    # Elimination of objects that are vertical and away from the center of the vehicle
    for c in contoursnew:
        if math.fabs(topmost[i][0] - centerImage[0]) > distFromCenter \
            and math.fabs(topmost[i][0] - bottommost[i][0]):
            cv2.drawContours(threshYWC, [c], 0, black, 30)
            np.delete(distance, i)
        i += 1

    i = 0
    # Elimination of rectangles that are not headed to the center of the vehicle
    for c in contoursnew:
        if topmost[i][0] < centerImage[0] and bottommost[i][0] < centerImage[0]:
            if topmost[i][0] < bottommost[i][0]:
                #cv2.drawContours(threshYWC, [c], 0, black, 30)
                np.delete(contours, c)
                np.delete(distance, i)
        if topmost[i][0] > centerImage[0] and bottommost[i][0] > centerImage[0]:
            if topmost[i][0] > bottommost[i][0]:
                cv2.drawContours(threshYWC, [c], 0, black, 30)
    ...
    for j in range(len(distance)):
        wholeH, _ = triobj(bottommost[j][0], topmost[j][0], bottommost[j][1], distance[j], maxHeight, centerImage[0])
        if lowH <= wholeH <= highH:
            np.delete(distance, [i])
    ...
    for i in range(len(distance)):
        ...
        if cx[i] < centerImage[0]:
            _, alpha = triobj(bottommost[i][0], topmost[i][0], bottommost[i][1], distance[i], maxHeight, centerImage[0])
            if alpha > cxtemp or itmp == -1:
                cxtemp = alpha
                itmp = i
    ...
    if cxtemp != 0:
        if distance[itmp] < distThresh:  # If the right line is dotted (Black color)
            cv2.line(roi_image, tuple(topmost[itmp]), tuple(bottommost[itmp]), black, 15)
            right = "Dotted"
...
while 1:
    ...
    if frame is None or ch & 0xFF == ord('q'):
        break
    roi_image, roi_image_copy, roi_image_most, threshYW, yellow_mask, threshYWC, white_mask, img_gray,  current_status= pV(frame)
