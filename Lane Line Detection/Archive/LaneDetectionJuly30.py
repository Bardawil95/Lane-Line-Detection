...
def pV(image):
    ...
    for c in contours:
        ...
        if areaArr[i] < minAreaThresh:
            cv2.drawContours(threshYW, [c], 0, black, 20)
        else:
            cv2.drawContours(threshYW, [c], 0, white, 5)
    ...
    for c in contoursnew:
       ...
       # print ">>>[(", bottommost[i][0], ",", bottommost[i][1], ")(", topmost[i][0], ",", topmost[i][1], ")], DX=", x, " DY=", y, "Distance[i]=", distance[i]
        ...
        if areaArr2[i] > area2 or y < yThresh:  # Elimination of non-rectangular objects
            cv2.drawContours(threshYW, [c], 0, black, 30)
            np.delete(distance, i)
        else:
            cv2.drawContours(threshYW, [c], 0, white, 3)
        i += 1

    kernal = np.ones((20, 20),np.int8)
    threshYW = cv2.morphologyEx(threshYW,cv2.MORPH_CLOSE,kernal)
    ...
    if cxtemp != 0:
        if distance[itmp] < distThresh:  # If the left line is dotted (Black color)
            ...
            left = "Dotted"
            y = bottommost[itmp][1] - topmost[itmp][1]  # y-coordinate distance for object
            x = bottommost[itmp][0] - topmost[itmp][0]  # x-coordinate distance for object
            '''
            print ">>>[(", bottommost[itmp][0], ",", bottommost[itmp][1], ")(", topmost[itmp][0], ",", \
            topmost[itmp][1], ")] DX=", x, " DY=", y, "Distance[i]=", distance[itmp]
            '''
            #print "Dotted Distance = ", distance[itmp]
        else:  # If the left line is continuous (Blue color)
            cv2.line(roi_image, tuple(topmost[itmp]), tuple(bottommost[itmp]), blue, 15)
            left = "Continuous"
            #print "Continuous Line, Distance = ", distance[i]

            y = bottommost[i][1] - topmost[i][1]  # y-coordinate distance for object
            x = bottommost[i][0] - topmost[i][0]  # x-coordinate distance for object
            '''
            print ">>>[(", bottommost[itmp][0], ",", bottommost[itmp][1], ")(", topmost[itmp][0], ",", topmost[itmp][1], \
            ")] DX=", x," DY=", y, "Distance[i]=", distance[itmp]
            '''
            #print "Continuous Distance = ", distance[itmp]
    else:
        print "None"
    ...
    if cxtemp2 != 0:
        if distance[itmp2] < distThresh:  # If the right line is dotted (Black color)
            ...
            #print "Dotted Line, Distance = ", distance[i]
            right = "Dotted"
    ...
    #textDisplay(roi_image, position(right, left))
    ...
    '''
    '''
    if left and right == "dotted":
        textDisplay(roi_image, 'Car is in the middle')
        print "Middle"
    else:
        if left == "continuous" and right == "dotted":
            print "left"
        else:
            textDisplay(roi_image, 'Car is in the extreme right')
            print "right"
    '''
    '''
    threshCanny = cv2.Canny(threshYW, 50, 150)
    line_image = 0
    output = sip.weighted_img(line_image, image, p1=0.8, p2=1., p3=0.)
    '''

    return roi_image, roi_image_most, threshYW
...
while 1:
    ...
    if frame is None or ch & 0xFF == ord('q'):
        break
    roi_image, roi_image_most, threshYW= pV(frame)
    threshYW = rescale_frame(threshYW, 60)
    roi_image = rescale_frame(roi_image, 60)
    cv2.imshow("Merged", threshYW)
