...
def pV(image):
    ...
    for c in contours_step5:
        ...
        centerX_step6.append((bottommost_step4[i][0] + topmost_step4[i][0]) / 2)  # x-coordinate center of object
        i += 1
    ...
    for c in contours_step6:
        ...
        centerX_step7.append((bottommost_step4[i][0] + topmost_step4[i][0]) / 2)  # x-coordinate center of object
        i += 1
    ...
    for c in contours_step7:
        ...
        if wholeH < lowH and wholeH > highH:
            cv2.drawContours(threshYWC, [c], 0, black, 30)
            continue
        ...
        centerX_step8.append((bottommost_step4[i][0] + topmost_step4[i][0]) / 2)  # x-coordinate center of object
    ...
    alphatemp = 0
    itmp = -1
    for i in range(len(distance_step8) - 1):
        ...
        if bottommost_step8[i][0] < centerWidth:  # the line is on the left
            _, alpha = triobj(bottommost_step8[i][0], topmost_step8[i][0], bottommost_step8[i][1], distance_step8[i],
                              maxHeight, centerWidth)
            print "temporary alpha = ", alphatemp, "alpha = ", alpha
    ...
    # Step 10: determine the location of the vehicles (Left, Right, Middle ot Unknonw)
